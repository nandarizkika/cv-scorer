    ...
    try:
        ...
        with cv_tab:
            ...
            if not cv_text or len(cv_text.strip()) < 100:
                st.warning("Insufficient CV text available to generate personalized questions.")
            else:
                try:
                    # Generate CV-based questions using the CV text
                    ...
                    if not current_heading and cv_questions:
                        # Just display the raw response
                        st.markdown(cv_questions)
...
def display_cv_results(cv_name, scores, _format_score=None):
    ...
    if 'processing' in st.session_state and 'processed_files' in st.session_state.processing:
        if cv_name in st.session_state.processing['processed_files']:
            ...
            print(f"Found CV text for {cv_name}: {len(cv_text_for_questions)} characters")
        else:
            print(f"CV text not found for {cv_name} in processed_files")
            # Try to use the CV name as a fallback
            cv_text_for_questions = f"Candidate CV: {cv_name}"
    else:
        print("Processed files dictionary not found in session state")
    
    # Add a horizontal rule to visually separate the sections
    st.markdown("<hr style='margin-bottom: 20px;'>", unsafe_allow_html=True)
    
    # Display interview questions - PASS THE ACTUAL CV TEXT HERE
    display_streamlit_interview_questions(scores, avg_score, cv_text_for_questions)
